...
class MaterialPurchaseRequisition(models.Model):
    ...
    def _prepare_pick_vals(self, line=False, stock_id=False):
        pick_vals = {
            ...
            #'location_dest_id' : self.purchase_order_ids[0].picking_type_id or self.dest_location_id.id,
...
#                     raise Warning(_('Select Destination location under the picking details.'))
                picking_vals = {
                        ...
                        'picking_type_id' : rec.custom_picking_type_id.id, #rec.custom_picking_type_id.id,#internal_obj.id,
                        'note' : rec.reason,
            ...
            for line in rec.requisition_line_ids:
                if line.requisition_type =='internal':
                    ...
                    if not rec.location_id.id:
                        raise Warning(_('Select Source location under the picking details.'))
                    if not rec.dest_location_id.id:
                        raise Warning(_('Select Destine Location under the picking details.'))
                    if not rec.custom_picking_type_id.id:
                        raise Warning(_('Select Type Picking under the picking details.'))
